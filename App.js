...
import { Location, DraggableMarker, Menu, PrimaryButtons, MarkerDescriptor } from '../ElemenD2/js/commons.js';
    ...
    <View style = {{flex: 1}}>
      ...
      >
        ...
        <SpriteMarker
          ...
          type = {marker.type}
          OnEnd = {event => marker.position = event.nativeEvent.coordinate}/>
        ))}
      </MapView>
      ...
      {primaryState && <PrimaryButtons 
        ...
        onAdd = {toogle}
        onSubmit = {() => UploadMarkers({ 
          type: "MarkerStorage",
          newJson: arr.map(x => x.GetObject()) 
        })}/>}
      {menuState && <Menu 
        elems = {menuElems}
